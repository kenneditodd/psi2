...
myContrast2 <- c("L.8h.M - S.8h.M", "H.8h.M - S.8h.M", "K.8h.M - S.8h.M",
...
# loop through dimensions and top genes
names <- v$targets$run_flowcell_lane
dims <- list(c(1,2),c(2,3),c(3,4))
topGenes <- c(100,500)
...
for (dim in dims) {
  ...
  for (numGenes in topGenes) {
    ...
    toplot <- data.frame(DimX = mds$x, 
                         DimY = mds$y,
                         Sample = rownames(mds$distance.matrix.squared),
                         batch = v$targets$run_flowcell_lane)
    ...
    p <- ggplot(toplot, aes(x = DimX, y = DimY, color = batch, label = Sample)) + 
      geom_point(size = 3) +
      theme_bw() +
      geom_text(aes(label = Sample), size=4, vjust = -1) +
      xlab(x.lab) + ylab(y.lab) + 
