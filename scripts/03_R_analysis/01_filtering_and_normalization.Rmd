...
```{r}
# extract data
df <- data.frame(sample = dge$samples$sample_name,
                 library = dge$samples$lib.size,
                 group = dge$samples$group)
  
# plot
p <- ggplot(data = df, mapping = aes(x = sample, y = library, fill = group)) +
  geom_col() +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
...
# save
path <- paste0(out,"prefiltering_QC/library_size_bar_graph_raw")
saveToPDF(paste0(path, ".pdf"), width = 25, height = 6)
dev.off()
```
...
remove(meta,mean.group,cpm,keep,start,end,mean.group.final,strict)
```
